...
modelling (ABM) --- a workhorse across ecology, epidemiology, the social
sciences and the geosciences --- is typically practised on platforms (NetLogo,
Mesa) that rely on global, unseeded randomness and offer no guarantee that a
result can be reproduced bit-for-bit, let alone across machines or as
parallelism is added. We present swarm-abm, a spatial ABM engine in Rust built
around \emph{determinism by construction}. A single agent/model API spans three
spatial paradigms (grid, network, continuous space) over a seedable, portable
generator. The engine's central mechanism is a two-phase synchronous update
whose decision phase receives the model immutably: the type system proves that
no agent mutates shared state during decision, which simultaneously (i)
eliminates the read-after-write hazard of synchronous ABM by construction ---
a verification guarantee rather than a convention --- and (ii) makes the
decision phase safe to parallelise. Paired with a per-agent generator keyed on
$(\text{seed},\text{step},\text{agent})$, parallel execution is bit-for-bit
identical to sequential, independent of thread count. We validate the generator
with PractRand and confirm identical results across x86-64 and WebAssembly. On
spatial SIR and Schelling, swarm-abm runs 2--5$\times$ faster single-threaded
than Agents.jl --- the fastest compiled ABM framework --- and 45--184$\times$
faster than Mesa; the parallel decision phase scales $\sim$5$\times$ on 16
cores for compute-bound decisions. A real debris-flow application shows how this
throughput turns an infeasible study --- a statistically powered comparison of
five metaheuristic calibrators --- into a routine one. Python bindings and a
WebAssembly viewer are provided; the engine is open source.
\end{abstract}
...
mirrors of Schelling segregation and spatial SIR in Mesa and compared the two
...
similarity (which depends on the exact spatial configuration) and Sugarscape's
Gini coefficient. Identity holds despite the change in pointer width
...
Table~\ref{tab:crossengine} reports per-step cost on spatial SIR and Schelling.
...
\begin{table}[t]
\centering
\caption{Cross-engine cost (ms/step, median of 3 seeds). Speedups are
swarm-abm relative to each comparator.}
\label{tab:crossengine}
\begin{tabular}{llrrrrr}
\toprule
Model & Grid & swarm-abm & Agents.jl & Mesa & vs Agents.jl & vs Mesa \\
\midrule
SIR & $25^2$  & 0.016 & 0.058 & 0.87  & 3.6 & 54 \\
SIR & $50^2$  & 0.067 & 0.275 & 3.55  & 4.1 & 53 \\
SIR & $100^2$ & 0.246 & 1.287 & 14.6  & 5.2 & 59 \\
SIR & $200^2$ & 1.339 & 6.619 & 64.6  & 4.9 & 48 \\
\midrule
Schelling & $25^2$  & 0.052 & 0.121 & 2.32  & 2.3 & 45  \\
Schelling & $50^2$  & 0.176 & 0.562 & 8.95  & 3.2 & 51  \\
Schelling & $100^2$ & 0.838 & 2.639 & 54.8  & 3.1 & 65  \\
Schelling & $200^2$ & 3.388 & 12.90 & 621.9 & 3.8 & 184 \\
\bottomrule
\end{tabular}
\end{table}
...
per-agent decision cost (Table~\ref{tab:scaling}). When decisions are
compute-bound --- the regime of agents with expensive logic (utility
maximisation, perception, local optimisation), common in economic and social
...
1-thread parallel run). Compute- versus memory-bound per-agent decisions.}
\label{tab:scaling}
\begin{tabular}{lrrrrr}
\toprule
Regime & 1 & 2 & 4 & 8 & 16 \\
\midrule
Compute-bound & 1.00 & 1.68 & 2.57 & 3.89 & 5.27 \\
Memory-bound  & 1.00 & 1.08 & 1.27 & 1.48 & 1.63 \\
\bottomrule
\end{tabular}
\end{table}

Combined with the single-thread advantage over Agents.jl, the parallel decision
phase widens the gap precisely in the regime --- expensive per-agent decisions
--- where intra-step parallelism pays.
