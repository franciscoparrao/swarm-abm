...
\begin{itemize}
  ...
  Agents.jl \cite{datseris2022agents}, one of the fastest compiled ABM frameworks,
...
defect. In NetLogo, Mesa, Repast, Agents.jl and the Rust framework krABMaga
\cite{antelmi2024krabmaga} this correctness condition is maintained by user
discipline (writing to a separate ``next'' field, or reading from a hand-built
snapshot); nothing in the tooling prevents a violation.
...
remedy itself (double-buffering) is well known; what is new is that the type
system makes it mandatory rather than advisory, which is also what licenses the
safe parallelism of Section~\ref{sec:sync} below. We are precise about the scope
of the guarantee: the compiler prevents an agent from writing the model during
\texttt{decide}, but it does not certify that the snapshot the author publishes
captures everything a correct decision needs. The guarantee is the absence of
accidental shared-state writes, not a proof that the model's update logic is
itself correct. The
...
\end{enumerate}
We can state the resulting property as follows.
\begin{quote}
\emph{Property (deterministic parallelism).} For any model whose \texttt{decide}
reads only the environment and a snapshot the model publishes before the phase
(a precondition the type system enforces, by making the model immutable in
\texttt{decide}), and for any number of threads, the parallel run produces the
same state, bit-for-bit, as the sequential run.
\end{quote}
We verify it empirically on a stochastic model (Section~\ref{sec:vv}). In the
vocabulary of parallel and distributed simulation this property is
\emph{repeatability} \cite{fujimoto1990pdes,fujimoto2000pads}: identical outputs
for identical inputs, irrespective of the parallel schedule. Repeatability is a
long-studied and hard-won goal there, classically pursued through
synchronisation protocols or deterministic-replay machinery. What is distinctive
in swarm-abm is the \emph{mechanism}: repeatability follows by construction from
the type system, with no runtime protocol, and it holds bit-for-bit across
thread counts and (empirically) across platforms.
...
SIR in Mesa and compared the two engines distributionally over 50 replicates
per engine. The primary evidence is an effect-size bound: the ensemble-mean
trajectories of the two engines differ by less than $0.021$ across the whole
horizon, for every metric. Consistently, a two-sample $z$-test per summary
metric ($\alpha = 0.05$) finds no statistically detectable difference on any of
the seven metrics ($|z| \le 1.22$); we read the test as corroborating the small
effect-size bound, not as proof of equivalence in itself (a non-significant
test does not establish equality). Independently, the \texttt{diffuse}
operator converges to its analytic fixed point, validating the spatial update
...
coefficient of a Sugarscape model \cite{epstein1996growing}. Identity holds
despite the change in pointer width (64-bit \texttt{usize} natively versus
32-bit on wasm32). We stress that this cross-platform identity is an
\emph{empirical} result, not a construction-level guarantee like the
thread-count case of Section~\ref{sec:sync}: a model that sampled from ranges
wider than $2^{32}$ in a width-dependent way could in principle diverge, so
cross-platform bit-identity should be verified per model rather than assumed.
...
machine, and a like-for-like protocol. The machine is an Intel i7-1270P, a
hybrid design with 4 performance and 8 efficiency cores (12 cores, 16 threads);
we note the heterogeneity because it bears on the scaling results below. Because the
...
seeds. The comparators are Agents.jl 7.0.2 (Julia 1.10), one of the fastest
compiled ABM frameworks, and Mesa 3.5 (CPython 3.12). NetLogo, Repast HPC
...
single CPU node. To bound dispersion we repeated the SIR comparison over ten
seeds. Absolute per-step timings carry seed- and load-dependent variance
(interquartile range roughly 10--25\%), so we summarise the comparison by the
\emph{per-seed speedup ratio}, which cancels shared run conditions: the
swarm-abm-over-Agents.jl ratio on SIR has a median of $4.8\times$ (interquartile
range $4.5$--$5.3\times$, $n=10$) at $100^2$, confirming the margin is not an
artefact of few replicates.
...
We also compared against krABMaga \cite{antelmi2024krabmaga,antelmi2019rust},
the other performance-oriented Rust ABM framework, on the same spatial SIR and
Schelling models. Implemented with krABMaga's efficient scalar field, it is in
fact \emph{modestly faster} than swarm-abm on raw single-thread per-step cost,
by up to roughly $2\times$ across both models. We therefore make no raw-speed
claim over krABMaga; what swarm-abm adds relative to it is \emph{deterministic,
reproducible parallelism} and the compiler-verified synchronous update, neither
of which krABMaga provides (Section~\ref{sec:discussion}). The mirrors are in
the repository.

\subsection{Throughput across spatial paradigms}
The comparison above uses grid models; to check that the engine's generality is
not merely nominal, we measured single-thread throughput on the other two
paradigms. On a network (small-world SIR, $50\,000$ nodes) the engine sustains
on the order of $2\times10^7$ agent-steps per second, comparable to the grid; on
continuous space (Reynolds flocking, $20\,000$ boids) it sustains on the order
of $4\times10^5$, roughly fifty times lower. The gap is a property of the
models, not the engine: flocking performs a radius query and rebuilds a spatial
index every agent-step, far heavier than the fixed-degree neighbour lookups of
the grid and the network. All three paradigms run within the same engine; a
\emph{cross-engine} comparison on the network and continuous paradigms is left
to future work.
...
\emph{synthetic} per-agent decision cost that stands in for the decision logic
of more elaborate agents (Figure~\ref{fig:scaling}). When decisions are
...
Amdahl's law sets the ceiling: the apply phase is sequential. We measured its
cost and found it a small fraction of the step in the compute-bound regime where
parallelism pays most, so it does not bind the headline speedup there (it is more
visible for cheap, memory-bound decisions). The hybrid core layout also matters: beyond
eight threads the slower efficiency cores are engaged, so the curve reflects
core heterogeneity as well as Amdahl's law, and should not be read as a clean
strong-scaling result on homogeneous cores.
...
global generator, so its runs could not be replicated. We are explicit about
novelty here: the debris-flow model and its calibration results belong to the
separate manuscript cited above; what \emph{this} paper contributes is the
engine and the demonstration that its throughput makes the two studies below
feasible. The debris-flow results are not re-claimed as new.
...
\begin{itemize}
  ...
  $11$--$34$~h, two to three orders of magnitude faster. Crucially, the cheap
  runs let us calibrate
  ...
  \cite{demsar2006statistical}. Grey Wolf Optimisation ranks first and performs
  competitively out of sample. This is exactly the kind of rigorous comparison
  the cost of an interpreted engine ordinarily precludes.
\end{itemize}
...
correctness proof into a reproducibility guarantee. The kernel is realised here
with Rust's ownership and borrowing, but the principle is language-agnostic: it
asks only for a type discipline that can enforce a read-only view of the model
during the decision phase, which const-correctness, ownership or capability
systems can all provide. The contribution is the principle, not the language.
That swarm-abm trades some throughput, and single-node operation, for these
guarantees is a deliberate choice: an ABM whose results cannot be replicated
exactly, across machines or as it is scaled, is a weaker basis for inference, and
we take exact reproducibility to be worth that cost.
...
deterministic parallelism and a WebAssembly target. The closest comparator is
krABMaga \cite{antelmi2024krabmaga,antelmi2019rust}, a MASON-derived Rust
framework that, like swarm-abm, offers a fast core, intra-step parallelism and a
WebAssembly target, and that is mature in areas we do not address (distributed
model exploration, a rich visualiser), and is in fact modestly faster on raw
single-thread throughput (Section~\ref{sec:perf}). What distinguishes swarm-abm
is therefore not speed but \emph{guarantees}: krABMaga parallelises agent scheduling without
guaranteeing that the parallel result is reproducible, and, like the others,
leaves the synchronous-update correctness condition to the modeller. swarm-abm's
contribution is precisely to make both into guarantees: bit-identical parallel
execution and a compiler-verified synchronous update. swarm-abm is also
complementary to distributed and GPU engines: Repast HPC \cite{collier2013repast}
targets clusters and FLAME GPU \cite{richmond2023flamegpu} targets the GPU,
regimes we do not address. Standard model-description practice such as the ODD
protocol \cite{grimm2006odd} is orthogonal and compatible.
...
applies to the synchronous decision phase; the apply phase is sequential. We
explored parallelising it and found the trade-off less clean than it first
appears: an apply phase that is safe to parallelise must write only to the agent
itself, yet the immediate model-level writes of a general apply phase (collision
resolution between competing decisions, updates to a derived field) are exactly
what give synchronous activation its semantics. Keeping apply sequential
preserves those semantics, and (Section~\ref{sec:perf}) it is not the binding
cost for the models where parallelism matters most. Finally, while we report
single-engine throughput for all three spatial paradigms
(Section~\ref{sec:perf}), the \emph{cross-engine} comparison covers only the two
grid-based models (SIR and Schelling), so the margins over Agents.jl, Mesa and
krABMaga should not be assumed to transfer unchanged to the network and
continuous-space settings.
