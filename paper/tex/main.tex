...
safe parallelism of Section~\ref{sec:sync} below. The
...
\end{enumerate}
The resulting property is stated plainly: for any model expressible under the
parallel API, whose snapshot contract the type system enforces, and for any
thread count, the parallel run produces the same state, bit-for-bit, as the
sequential run. We verify it empirically on a stochastic model
(Section~\ref{sec:vv}). This is the payoff of the framing:
...
the other performance-oriented Rust ABM framework, on the same spatial SIR. The
two are in the same single-thread throughput ballpark: implemented with
krABMaga's efficient scalar field, it runs the model at roughly the same
per-step cost as swarm-abm (within a factor of two either way across the grid
sizes above). We therefore make no raw-speed claim over krABMaga; what swarm-abm
adds relative to it is \emph{deterministic, reproducible parallelism} and the
compiler-verified synchronous update, neither of which krABMaga provides
(Section~\ref{sec:discussion}). The mirror is in the repository.
...
model exploration, a rich visualiser). On raw single-thread throughput the two
are comparable (Section~\ref{sec:perf}). What distinguishes swarm-abm is not
speed but \emph{guarantees}: krABMaga parallelises agent scheduling without
